...
window.Vaadin.Flow['_vaadintheme_bugfinder_globalCss'] = window.Vaadin.Flow['_vaadintheme_bugfinder_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_bugfinder_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_bugfinder_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_bugfinder_globalCss'].push(target);
  }
  if (!document['_vaadintheme_bugfinder_componentCss']) {
    
    document['_vaadintheme_bugfinder_componentCss'] = true;
  }
